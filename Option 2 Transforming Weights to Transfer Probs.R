...
Edges_Stays$EdgeWeightOverStay=Edges_Stays$Edge_Weights/Edges_Stays$AnnualStays
Edges_Stays$EdgeWeightOverStay=Edges_Stays$Edge_Weights/Edges_Stays$AnnualStays
Edges_Stays$ThreeWeekStays=round(Edges_Stays$AnnualStays*(3/52))
Edges_Stays$ThreeWeekTransfers=round(Edges_Stays$Edge_Weights*(3/52))


#######################
#### Probabilities ####

ThreeWeek_Probabilities = foreach(i=1:nrow(Edges_Stays)) %do% (1-dbinom(0, size = Edges_Stays[i,"ThreeWeekStays"], prob = (Edges_Stays[i,"ThreeWeekTransfers"]+1)/Edges_Stays[i,"ThreeWeekStays"]))
...
ThreeWeek_Probabilities_Transformed = -log(unlist(ThreeWeek_Probabilities))

hist((ThreeWeek_Probabilities_Transformed))
range((ThreeWeek_Probabilities_Transformed), na.rm = T)
table(ThreeWeek_Probabilities_Transformed)

###############################
#### Check Distance Matrix ####

cat("Replace edge weights of Department network with transformed weights")
E(directed.graph_Dept)$weight=unlist(ThreeWeek_Probabilities_Transformed)
